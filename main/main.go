...
func main() {
	...
	db := fmt.Sprintf("postgres://postgres:2508@localhost:5432/postgres") //  подключение к базе данных

	conn, err := pgx.Connect(context.Background(), db)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Неподключено к БД: %v\n", err)
		os.Exit(1)
	}
	...
	sc, _ := stan.Connect("my-cluster", "client-1") // подключаемся к каналу натс
	defer sc.Close()
	...
	r := gin.Default()
	log.Println("Http сервер запущен, чтобы подключиться - http://localhost:8888")
	r.LoadHTMLGlob("wb.html")
	r.GET("/", func(c *gin.Context) {
		c.HTML(http.StatusOK, "wb.html", gin.H{
